...
def simplex_next_state(current_vector, input_vector, n_neighbors, library_ts, leak, tp = 1, bias = 0): #------------------------#
    ...
    t1 = leak @ t0 + ((I - leak) @ (input_vector + SimplexProjection(t0, library_ts, n_neighbors, tp = tp) + bias))
    return(t1)
#---------------------------------------------------------------------------#
...
class SimplexReservoir(): #=========================================================================#
    ...
    def prepare_target_data(self, target_ts, target_var, target_db_name, test_fraction, tp_pred = 1):
        ...
        self.train_true = std_data[self.tp_pred:(train_size + self.tp_pred)].reset_index()[self.target_var]
        self.test_data = std_data[train_size:(data_size - self.tp_pred)].reset_index()[self.target_var]
        self.test_true = std_data[(train_size + self.tp_pred):data_size].reset_index()[self.target_var]
    
    # Step 3: Initialize reservoir
    def initialize_reservoir(self, w_in_strength = 0.1, w_in_sparsity = 0.2, n_nn = None,
        ...
        np.random.seed(1234); W_in0 = np.random.uniform(-1, 1, (self.num_input_nodes * self.num_nodes, 1))
        np.random.seed(1234); rand_id = np.random.choice(W_in0.shape[0], int(self.num_input_nodes * self.num_nodes * self.w_in_sparsity), replace = False)
        W_in0[rand_id] = 0
        self.W_in = W_in0.reshape(self.num_input_nodes, self.num_nodes) * self.w_in_strength
